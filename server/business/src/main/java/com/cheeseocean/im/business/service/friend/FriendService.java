 ...
 * 好友关系服务实现。
 *
 * <p>通过 {@link FriendRepository} 聚合仓储完成好友申请、好友关系、黑名单的业务逻辑，
...
public class FriendService implements FriendRelationService {
    ...
    public List<FriendRequestSummary> listIncomingRequests(String userId) {
        return friendRepository.listIncomingPendingRequests(userId).stream()
                .map(req -> toRequestSummary(req.getFromUserId(), "incoming", "pending", req.getReqMsg()))
                .toList();
    }

    @Override
    public List<FriendRequestSummary> listOutgoingRequests(String userId) {
        return friendRepository.listOutgoingPendingRequests(userId).stream()
                .map(req -> toRequestSummary(req.getToUserId(), "outgoing", "pending", req.getReqMsg()))
                .toList();
    }
    ...
    public FriendRequestSummary sendFriendRequest(String userId, String friendUserId, String requestMessage) {
        ...
        if (friendRepository.hasIncomingPendingRequest(userId, friendUserId)) {
            throw new IllegalStateException("incoming friend request pending; accept instead");
        }
        Optional<FriendRequest> existing = friendRepository.findPendingRequest(userId, friendUserId);
        if (existing.isEmpty()) {
            ...
            // 已有待处理申请，直接返回已有记录的附言
            requestMessage = existing.get().getReqMsg();
        }
    ...
    public FriendRequestSummary rejectFriendRequest(String userId, String friendUserId) {
        ...
        friendRepository.rejectRequest(friendUserId, userId);
    ...
    public FriendRequestSummary cancelFriendRequest(String userId, String friendUserId) {
        if (!friendRepository.hasOutgoingPendingRequest(userId, friendUserId)) {
            throw new IllegalStateException("friend request not found");
        }
        friendRepository.cancelRequest(userId, friendUserId);
