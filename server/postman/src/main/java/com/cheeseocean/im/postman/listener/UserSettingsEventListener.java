...
public class UserSettingsEventListener {
    ...
    public UserSettingsEventListener(OnlineRouteQueryService onlineRouteQueryService,
    ...
    @QueueListener(topic = TopicNames.USER_SETTINGS, group = "push-user-settings")
    public void onMessage(UserSettingsEvent event) {
        try {
            handle(event);
        } catch (Exception e) {
            log.error("Failed to handle user settings event: {}", event, e);
        }
    }

    void handle(UserSettingsEvent event) {
    ...
    private DispatchPayload toDispatchPayload(UserSettingsEvent event) {
        DispatchPayload payload = new DispatchPayload();
        payload.setServerMsgId("user-settings:" + event.getRecipientUserId() + ":" + event.getOccurredAt());
        payload.setConversationId("user:settings:" + event.getRecipientUserId());
        payload.setContentType(0);
        payload.setContent("refresh");
        payload.setSendTime(event.getOccurredAt());
        payload.getExt().put("notificationType",    "user_global_recv_msg_opt_changed");
        payload.getExt().put("globalRecvMsgOpt",    String.valueOf(event.getGlobalRecvMsgOpt()));
