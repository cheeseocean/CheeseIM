...
public class DeliveryTaskListener {
    ...
    @KafkaListener(topics = KafkaTopics.Message.DELIVERY, groupId = "postman-delivery")
    ...
    void handle(DeliveryTaskCommand task) {
        ...
        if (!pushResult.isRouteFound() || pushResult.getDeliveredDeviceIds().isEmpty()) {
            kafkaTemplate.send(KafkaTopics.Message.OFFLINE_PUSH, task.deliveryKey(), OfflinePushTask.from(task));
