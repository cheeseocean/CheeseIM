...
public class KafkaQueueAdapter implements QueueAdapter {
    ...
    public KafkaQueueAdapter(KafkaTemplate<String, byte[]> kafkaTemplate,
        ...
        this(kafkaTemplate, objectMapper, kafkaProperties, new QueueProperties());
    }
    ...
    /**
     * 反序列化策略与 {@link com.cheeseocean.im.infra.queue.chronicle.ChronicleQueueAdapter#deserialize}
    ...
    DefaultErrorHandler createErrorHandler() {
        QueueProperties.Consumer consumer = queueProperties.getConsumer();
        return createErrorHandler(new FixedBackOff(
                consumer.getRetryIntervalMillis(),
                Math.max(0, consumer.getMaxAttempts() - 1L)));
    }
