 ...
 * <p>定义入群申请领域对象的持久化操作。
 */
public interface GroupApplicationRepository {

    /** 查询指定用户对指定群的申请，不存在时返回 empty */
    Optional<GroupApplication> findByUserAndGroup(String userId, String groupId);

    /** 查询群内待处理的申请列表（按申请时间倒序） */
    List<GroupApplication> findPendingByGroupId(String groupId);

    /** 查询用户发出的待处理申请列表 */
    List<GroupApplication> findPendingByUserId(String userId);

    /** 保存（新增或覆盖）申请记录 */
    void save(GroupApplication application);
