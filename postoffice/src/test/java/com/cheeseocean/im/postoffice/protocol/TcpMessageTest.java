...
public class TcpMessageTest {
    ...
    private static final Logger logger = LoggerFactory.getLogger(TcpMessageTest.class);
    
    @Test
    public void testMagicValue() {
        // 验证MAGIC值是有效的十六进制数
        short magic = TcpMessage.MAGIC;
    ...
    public void testMessageEncodeAndDecode() {
        ...
        TcpMessage originalMessage = new TcpMessage(TcpMessageType.TCP_HEARTBEAT_REQ, operationID, testData);
        
        // 编码消息
        byte[] encodedBytes = originalMessage.encode();
        assertNotNull(encodedBytes);
        assertTrue(encodedBytes.length >= TcpMessage.HEADER_LENGTH);
        ...
        assertEquals(TcpMessage.MAGIC, encodedMagic);
        
        // 解码消息
        TcpMessage decodedMessage = TcpMessage.decode(encodedBytes);
    ...
    public void testMessageTypes() {
        // 测试各种消息类型
        testMessageType(TcpMessageType.TCP_CONNECT_REQ, "连接请求");
        testMessageType(TcpMessageType.TCP_AUTH_REQ, "认证请求");
        testMessageType(TcpMessageType.TCP_HEARTBEAT_REQ, "心跳请求");
        testMessageType(TcpMessageType.TCP_SEND_MSG_REQ, "发送消息请求");
    }
    ...
    private void testMessageType(byte msgType, String description) {
        ...
        TcpMessage message = new TcpMessage(msgType, operationID, testData);
    ...
    public void testEmptyData() {
        ...
        TcpMessage message = new TcpMessage(TcpMessageType.TCP_HEARTBEAT_REQ, operationID, null);
        byte[] encoded = message.encode();
        TcpMessage decoded = TcpMessage.decode(encoded);
        
        assertEquals(TcpMessageType.TCP_HEARTBEAT_REQ, decoded.getMsgType());
        assertEquals(operationID.trim(), decoded.getOperationID().trim());
    ...
    public void testLongOperationID() {
        ...
        TcpMessage message = new TcpMessage(TcpMessageType.TCP_SEND_MSG_REQ, longOperationID, testData);
        byte[] encoded = message.encode();
        TcpMessage decoded = TcpMessage.decode(encoded);
    ...
    public void testStaticFactoryMethods() {
        ...
        TcpMessage connectSuccess = TcpMessage.connectSuccess(operationID);
        assertEquals(TcpMessageType.TCP_CONNECT_SUCCESS, connectSuccess.getMsgType());
        assertEquals(operationID, connectSuccess.getOperationID());
        
        TcpMessage connectFailed = TcpMessage.connectFailed(operationID, "连接失败");
        assertEquals(TcpMessageType.TCP_CONNECT_FAILED, connectFailed.getMsgType());
        assertEquals("连接失败", connectFailed.getData());
        
        TcpMessage authSuccess = TcpMessage.authSuccess(operationID, "user123");
        assertEquals(TcpMessageType.TCP_AUTH_SUCCESS, authSuccess.getMsgType());
        assertTrue(authSuccess.getData().contains("user123"));
        
        TcpMessage heartbeatResp = TcpMessage.heartbeatResp(operationID);
        assertEquals(TcpMessageType.TCP_HEARTBEAT_RESP, heartbeatResp.getMsgType());
    ...
    public void testInvalidMagic() {
        // 测试无效的MAGIC值
        byte[] invalidMessage = new byte[TcpMessage.HEADER_LENGTH];
        ...
        // 设置其他必要字段
        invalidMessage[2] = TcpMessage.VERSION;
        invalidMessage[3] = TcpMessageType.TCP_HEARTBEAT_REQ;
        // Length = 0
        invalidMessage[4] = 0;
        invalidMessage[5] = 0;
        invalidMessage[6] = 0;
        ...
        assertThrows(IllegalArgumentException.class, () -> {
            TcpMessage.decode(invalidMessage);
        });
