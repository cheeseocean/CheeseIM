        ...
        try {
            ...
            if (magic != TcpMessage.MAGIC) {
                logger.warn("Invalid magic number: 0x{}, expected: 0x{}", 
                           ...
                           Integer.toHexString(TcpMessage.MAGIC & 0xFFFF));
            ...
            if (version != TcpMessage.VERSION) {
                logger.warn("Unsupported protocol version: {}, expected: {}", 
                           version, TcpMessage.VERSION);
                in.resetReaderIndex();
                in.readByte();
                return;
            }
            ...
            // 验证数据长度
            if (dataLength < 0 || dataLength > TcpMessage.MAX_DATA_LENGTH) {
                logger.warn("Invalid data length: {}, max allowed: {}", 
                           dataLength, TcpMessage.MAX_DATA_LENGTH);
                in.resetReaderIndex();
                in.readByte();
                return;
            }
            ...
            // 检查是否有足够的字节读取完整消息
            int totalLength = TcpMessage.HEADER_LENGTH + dataLength;
