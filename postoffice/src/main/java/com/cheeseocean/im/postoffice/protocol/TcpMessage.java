...
public class TcpMessage implements Serializable {
    ...
    public TcpMessage(byte msgType, String operationID, String data) {
        this();
     ...
     * 从字节数组解码消息
     */
    public static TcpMessage decode(byte[] bytes) {
     ...
     * 从WebSocket消息创建TCP消息
     */
    public static TcpMessage fromWSMessage(WSMessage wsMessage) {
        if (wsMessage == null) {
            return null;
        }
        ...
        if (wsMessage.getMsgType() != null) {
            tcpMessage.msgType = TcpMessageType.wsToTcpMessageType(wsMessage.getMsgType());
        } else {
            tcpMessage.msgType = TcpMessageType.TCP_ERROR_RESP;
        }

        tcpMessage.operationID = wsMessage.getOperationID() != null ? wsMessage.getOperationID() : "unknown";
        tcpMessage.timestamp = wsMessage.getSendTime() != null ? wsMessage.getSendTime() : System.currentTimeMillis();

        // 将数据转换为JSON字符串
        if (wsMessage.getData() != null) {
            try {
                if (wsMessage.getData() instanceof String) {
                    tcpMessage.data = (String) wsMessage.getData();
                } else {
                    // 这里可以使用Jackson等JSON库序列化
                    tcpMessage.data = wsMessage.getData().toString();
                }
                tcpMessage.dataLength = tcpMessage.data.getBytes(StandardCharsets.UTF_8).length;
            } catch (Exception e) {
                tcpMessage.data = "{}";
                tcpMessage.dataLength = 2;
            }
        } else {
            tcpMessage.data = null;
            tcpMessage.dataLength = 0;
        }
    ...
    public static TcpMessage connectSuccess(String operationID) {
        return new TcpMessage((byte) TcpMessageType.TCP_CONNECT_SUCCESS, operationID, "连接成功");
    }
    
    public static TcpMessage connectFailed(String operationID, String reason) {
        return new TcpMessage((byte) TcpMessageType.TCP_CONNECT_FAILED, operationID, reason);
    }
    
    public static TcpMessage authSuccess(String operationID, String userID) {
        return new TcpMessage((byte) TcpMessageType.TCP_AUTH_SUCCESS, operationID, 
                             "{\"userID\":\"" + userID + "\",\"message\":\"认证成功\"}");
    }
    
    public static TcpMessage authFailed(String operationID, String reason) {
        return new TcpMessage((byte) TcpMessageType.TCP_AUTH_FAILED, operationID, reason);
    }
    
    public static TcpMessage heartbeatResp(String operationID) {
        return new TcpMessage((byte) TcpMessageType.TCP_HEARTBEAT_RESP, operationID, "pong");
    }
    
    public static TcpMessage errorResp(String operationID, int errorCode, String errorMsg) {
        return new TcpMessage((byte) TcpMessageType.TCP_ERROR_RESP, operationID, 
